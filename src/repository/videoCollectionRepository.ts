...
import videoHistorySchema, { VideoHistory as VideoHistoryType } from "../model/schema/videoHistorySchema";
import watchLaterSchema, { IWatchLater as WatchLaterType } from "../model/schema/watchLaterSchema";
...
export class VideoCollectionRepository implements IVideoCollectionRepository {
  ...
  async addToHistory(userId: string, videoData: any): Promise<VideoHistoryType> {
    const isVideoExists = await this.isVideoInHistory(userId, videoData.videoId);
  ...
  async removeFromHistory(userId: string, videoId: string): Promise<VideoHistoryType> {
    const history = await videoHistorySchema.findOneAndUpdate(
      { userId },
      { $pull: { videos: { videoId } } },
      { new: true }
  ...
  async createHistory(userId: string, videoData: any): Promise<VideoHistoryType> {
    return await videoHistorySchema.create({ userId, videos: [videoData] });
  }

  async getHistory(userId: string, limit: number, skip: number): Promise<VideoHistoryType> {
  ...
  async addToWatchLater(userId: string, videoId: string): Promise<WatchLaterType> {
  ...
  async removeFromWatchLater(userId: string, videoId: string): Promise<WatchLaterType> {
    const watchLater = await watchLaterSchema.findOneAndUpdate(
      { userId },
      { $pull: { videos: { videoId } } },
      { new: true }
  ...
  async createWatchLater(userId: string, videoId: string): Promise<WatchLaterType> {
    return await watchLaterSchema.create({ userId, videos: [{ videoId }] });
  }

  async getWatchLater(userId: string, limit: number, skip: number): Promise<WatchLaterType> {
